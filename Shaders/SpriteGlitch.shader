...
uniform float color_strength = 0.1;
...
void fragment() {
  ...
  texColor = texColor + (vec4(-0.5)+vec4(rand(vec2(fragCoord.y,time)),rand(vec2(fragCoord.y,time+1.0)),rand(vec2(fragCoord.y,time+2.0)),0))*color_strength;
